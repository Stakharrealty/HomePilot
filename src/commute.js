...
//
// Contains: DRIVE_TABLE, DRIVE_TO_TORONTO, FSA_TO_WORK_ZONE, CITY_TO_WORK_ZONE,
// getWorkZone(), SUBZONE_ADJUST, calcCommuteMinutes(), ACCESS_TIERS,
...
function calcCommuteMinutes(cityName) {
  ...
  else multiplier = 1.55;                     // long haul, worst compounding
  // Past two hours, only the first two hours are rush-hour driving; the rest is
  // open highway at the off-peak pace. CHANGED 2026-09-23: this used to be a
  // flat x1.10 on the whole trip, which made the curve run backwards at the
  // join -- a 120-minute drive showed 186 while a 130-minute drive showed 143,
  // so Welland looked further from Toronto than Collingwood. Up to 120 minutes
  // nothing changes.
  if(baseMin > 120) return Math.round(120 * multiplier + (baseMin - 120));
  return Math.round(baseMin * multiplier);
}

// getCommuteScore() was REMOVED 2026-09-23 (IMPROVEMENT_PLAN.md 1.4). It turned
// minutes into a 0-100 score that a weighted sum blended with affordability,
// and it bottomed out at 3 for anything over 120 minutes -- so no commute,
// however long, could ever rule a city out. That is how a hybrid Toronto
// worker got Welland (186 min each way) ranked #1. Commute is now the buyer's
// own hard limit (maxCommuteMin in main.js, applied in ranking.js).

// ─── WORK ACCESS TIER ───────────────────────────────────────────────────────
// CHANGED 2026-09-23 (IMPROVEMENT_PLAN.md 1.4). This used to say nothing in the
// UI should show a raw minute count, only these four tiers. Two things changed
// that. The buyer now sets their own limit in minutes ("longest commute you'd
// accept"), and has to be able to see the number it is compared against. And
// "Limited Commute" covered everything from 71 minutes to nearly eight hours,
// which hid exactly the difference that mattered. Cards now show the estimate
// rounded to 5 minutes and labelled as an estimated drive (see
// commuteEstimateMin() in ranking.js). The figures are still estimates, not
// measurements; the rounding and the label say so. The tiers remain for the
// AI insights and the report.
const ACCESS_TIERS = [
