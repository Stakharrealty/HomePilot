// HomePilot Regression Suite â rebuilt July 9, 2026 to the documented 6-suite spec.
...
// exactly as the live page does â external ctx.x assignment does not reach these.
...
// âââââââââââââââââââââââââââââ SUITE 1: CORE âââââââââââââââââââââââââââââ
suite('Core');
{
  ...
  t('BP 250k/150k in plausible range (0.9Mâ1.4M)', bp.bp>900000 && bp.bp<1400000);
...
}
{
  ...
  t('tax = price Ã city rate (Brampton 1.039%)', Math.abs(cc.tax - Math.round(540000*0.01039/12)) <= 1);
  ...
  t('zero loan â zero mortgage', run(`calcCosts(M.find(c=>c.n==='Brampton'),500000,'3',500000,'detached')`).mort === 0);
...
}
{
  ...
  for(const [n,p,tp] of cases){
    ...
    t('ins '+n+' detached in $1.1kâ2.6k/yr band', c.ins*12>=1100 && c.ins*12<=2600);
  }
  const tc = run(`calcCosts(M.find(c=>c.n==='Toronto - Downtown'), 635000, '3', 127000, 'condo')`);
  t('ins Toronto condo in $300â800/yr band', tc.ins*12>=300 && tc.ins*12<=800);
}
{
...
}
{
  ...
  t('net income plausible (55â82% of gross)', net/250000 > 0.55 && net/250000 < 0.82);
...
}
{
  ...
  if(rw){
    ...
    t('RANKING_WEIGHTS not a separate global (inlined in computeCityScore â verified via behavior, not a bug)', true);
    t('weights cover daily/hybrid/remote (behavioral proxy passed above)', true);
    t('weights sum to 1 (n/a â inlined)', true);
...
suite('Explainability');
{
  ...
  t('tier monotonic: 10 â  90', at10.label !== at90.label);
  ...
  t('burden calc sane (5â60%)', burden>=5 && burden<=60);
  ...
  t('thresholds universal: same ratio (36%) â same label at different income levels', sameRatioA === sameRatioB);
...
suite('Commute');
{
  ...
  t('BramptonâBrampton short (<25min)', cmB < 25);
  ...
  t('commute score bounded (0â100)', [s10,s45,s120].every(v=>v>=0 && v<=100));
...
suite('AnglePicks');
{
  ...
  for(const [inc,dn,wa] of profiles){
    ...
    if(picks === null){
      ...
      `);
      t('picks structured @'+inc+' (null correctly triggers Stretch fallback â nothing under 45% burden)', genuinelyUnaffordable);
      t('no dup cities @'+inc+' (n/a â null)', true);
      t('bestOverall valid @'+inc+' (n/a â null, Stretch fallback path)', true);
      t('mostHouse burden<45% @'+inc+' (n/a â null, Stretch fallback path)', true);
      continue;
    }
...
// âââââââââââââââââââââââââââââ SUITE 7: LEAD DELIVERY & BREAKDOWN âââââââââââââââââââââââââââââ
// Added July 16, 2026 â covers the same-day lead-delivery rewrite (Formspree -> Cloudflare
...
async function runSuite7(){
  ...
  // of simulating a full render we lock in the exact source lines â this catches any
  ...
  // the current rate â formalizes the manual Shelburne hand-check into a permanent test.
...
runSuite7().then(async () => {
  // âââââââââââââââââââââââââââââ SUITE 8: CARD LAYOUT âââââââââââââââââââââââââââââ
  // Added July 16, 2026 â locks in card-level layout decisions that have already
  ...
  // Structural source checks, not DOM rendering â consistent with the breakdown
  ...
  suite('CardLayout');
  {
    ...
    // textually after the opening tag while still nested inside it) â i.e. there
    ...
    t('.bk closes (at least one </div>) before view-btn â confirms the link sits OUTSIDE the collapsed section, not just after .bk\'s opening tag', closesBetween >= 1);
    ...
    // SHOULD require a click â only view-btn was the regression, not this).
    ...
    // Per-type View Available Homes links (added same day) â each accordion panel
  ...
  // âââââââââââââââââââââââ SUITE 9: AI INSIGHTS REDESIGN âââââââââââââââââââââââ
  // Added July 16, 2026 â locks in the AI insights redesign that removed the
  ...
  suite('AIInsights');
  {
    ...
    // HomePilot Score display â removed twice now (once before this session, once
  ...
  }
  {
    ...
    const fakeAnthropicResponse = {
      content: [{ type:'text', text: JSON.stringify({
        ...
        lifestyleSnapshot: 'TESTMARK_LIFESTYLE Suburban Â· Car-dependent'
      })}]
    };
  ...
  console.log('\nââââââââ HOMEPILOT REGRESSION â '+new Date().toISOString().slice(0,10)+' ââââââââ');
  for(const s of suiteResults) console.log((s.pass===s.total?'â':'â')+' '+s.name.padEnd(20)+' '+s.pass+'/'+s.total);
  console.log('ââââââââââââââââââââââââââââââââ');
  console.log('TOTAL: '+pass+'/'+(pass+fail)+(fail?'  â '+fail+' FAILURES':'  â ALL GREEN'));
  if(failures.length){ console.log('\nFAILURES:'); failures.forEach(f=>console.log('  â '+f)); }
